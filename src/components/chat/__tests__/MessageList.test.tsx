...
test("MessageList applies correct styling for user vs assistant messages", () => {
  ...
  expect(userMessage?.className).toContain("from-indigo-500");
...
test("MessageList shows loading for assistant message with empty parts", () => {
  ...
  const generatingElements = container.querySelectorAll(".text-indigo-600");
  const matches = Array.from(generatingElements).filter((el) =>
